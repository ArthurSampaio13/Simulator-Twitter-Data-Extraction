...
if __name__ == "__main__":
    ...
    with DAG(dag_id= "TwitterTest", 
        ...
        to = TwitterOperator(file_path=join("datalake/twitter_datascience",
                                           ...
                                           f"datascience {datetime.now().date().strftime('%Y%m%d')}.json"), query=query, start_time=start_time, end_time= end_time, task_id="teste_run")
        ti = TaskInstance(to)
        to.execute(context=ti.task_id)